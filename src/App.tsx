...
export default function App() {
  ...
  const [language, setLanguage] = useState(() => preference('shengji-language', 'zh'));
  const [autoTranscribe, setAutoTranscribe] = useState(() => preference('shengji-auto', 'true') === 'true');
  ...
  const restoredElapsed = useRef(0);
  ...
  useEffect(() => { try { localStorage.setItem('shengji-language', language); localStorage.setItem('shengji-auto', String(autoTranscribe)); } catch { /* Recording and export still work when preferences cannot be stored. */ } }, [language, autoTranscribe]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        recordingsRef.current = saved; setRecordings(saved);
        if (android) {
          const status = await NativeRecorder.status();
          if (cancelled) return;
          if (status.recording) { restoredElapsed.current = status.duration; activeMode.current = status.mode; setMode(status.mode); setElapsed(status.duration); setPhase('recording'); }
          else if (status.processing) setPhase('stopping');
          else { for (const item of (await NativeRecorder.pending()).recordings) { if (!cancelled) await nativeResultRef.current(item); } }
        }
      } catch { if (!cancelled) setNotice('无法读取本地资料库。请检查设备存储权限或剩余空间。'); }
    })();
  ...
  useEffect(() => {
    ...
    const begun = Date.now() - restoredElapsed.current * 1000;
  ...
  async function persist(next: Recording) {
    ...
    try { await saveRecording(next); return true; } catch { setNotice('设备空间不足或本地保存失败。当前内容仍在界面中，请立即导出录制文件和逐字稿。'); return false; }
  }
  async function runTranscription(item: Recording) {
    ...
    try {
      ...
      const saved = await persist({ ...current, segments, status: 'done', error: undefined });
      if (!saved) return;
      ...
      else { const message = errorText(error); const saved = await persist({ ...(recordingsRef.current.find(r => r.id === item.id) || item), status: 'error', error: message }); if (saved) setNotice(message); }
    } finally { abort.current = null; setProgress({ id: '', value: 0, message: '' }); }
  }
  async function finish(result: CaptureResult, recordedMode: Recording['mode'], options: { title?: string; id?: string; onSaved?: () => Promise<void> } = {}) {
    capture.current = null;
    const item: Recording = { ...result, id: options.id || crypto.randomUUID(), createdAt: new Date().toISOString(), title: (options.title ?? titleRef.current).trim() || `${recordedMode === 'screen' ? '屏幕录制' : recordedMode === 'import' ? '导入音频' : '语音记录'} ${new Date().toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}`, mode: recordedMode, segments: [], status: 'pending' };
    const saved = await persist(item); setSelectedId(item.id); setTitle('');
    if (saved && options.onSaved) await options.onSaved().catch(() => setNotice('录制已入库，原生临时文件将在下次打开应用时尝试清理。'));
    setPhase('idle'); stopping.current = false;
    if (!saved) return; // Keep the storage warning visible; originals are only in memory.
  ...
  async function consumeNative(result: NativeRecording) {
    ...
    try {
      ...
      const id = `native-${result.path.split('/').pop()}`;
      const existing = recordingsRef.current.find(r => r.id === id);
      if (existing && await saveRecording(existing).then(() => true, () => false)) { await NativeRecorder.acknowledge({ path: result.path }); setPhase('idle'); return; }
      await finish({ media: new Blob([media], { type: result.mimeType }), audio, mimeType: result.mimeType, duration: result.duration }, result.mode, { id, onSaved: () => NativeRecorder.acknowledge({ path: result.path }) });
    } catch (error) { nativeSeen.current.delete(result.path); stopping.current = false; setPhase('idle'); setNotice(errorText(error)); }
  }
  nativeResultRef.current = consumeNative;
  async function stop() {
    if (phase !== 'recording' || stopping.current) return;
    ...
    } catch (error) { setNotice(errorText(error)); setPhase('idle'); capture.current = null; stopping.current = false; }
  }
  stopRef.current = () => { void stop(); };
  async function begin(sourceId?: string) {
    if (recording || busy) return;
    setNotice(''); setPhase('starting'); setSelectedId(undefined); setElapsed(0); restoredElapsed.current = 0; stopping.current = false; activeMode.current = mode;
    try {
  ...
  async function importFile(file: File | undefined) {
    ...
    const importTitle = file.name.replace(/\.[^.]+$/, '');
    setPhase('stopping');
    let duration = 0;
    try {
      ...
      await finish({ media: file, audio: file, mimeType: file.type || 'audio/wav', duration }, 'import', { title: importTitle });
    } catch (error) { setPhase('idle'); setNotice(errorText(error)); }
  }
